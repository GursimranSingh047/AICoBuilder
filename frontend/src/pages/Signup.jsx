    ...
    <div className="min-h-screen bg-surface-DEFAULT dot-grid flex items-center justify-center px-4">
      <div className="w-full max-w-sm animate-slide-up">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-muted mt-1">Start building with AI</p>
        </div>
        ...
        <div className="card space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Username */}
            <div>
              <label className="text-xs font-mono text-muted uppercase tracking-wider mb-2 block">Username</label>
              <div className="relative">
                ...
                <div className="mt-2 space-y-1">
                  {checks.map(({ label, test }) => (
                    <div key={label} className="flex items-center gap-2">
                      <CheckCircle
                        size={12}
                        className={test(password) ? 'text-emerald' : 'text-surface-4'}
                      />
                      <span className={`text-xs ${test(password) ? 'text-emerald' : 'text-muted'}`}>
                        {label}
                      </span>
                    </div>
                  ))}
